/**
 * Curated high-value Singapore areas — the single source of truth shared by:
 ...
 *
 * Slugs are URL-safe and stable — they appear in the live sitemap and in
 * internal links, so don't rename one without a redirect.
 */
...
/** The N areas nearest to `area` by straight-line coordinate distance —
 *  used for "parking in nearby areas" cross-links on the SSR area pages. */
export function nearestAreas(area: SeoArea, n: number): SeoArea[] {
  return SEO_AREAS.filter((a) => a.slug !== area.slug)
    .map((a) => ({
      a,
      d: (a.lat - area.lat) ** 2 + (a.lng - area.lng) ** 2,
    }))
    .sort((x, y) => x.d - y.d)
    .slice(0, n)
    .map((x) => x.a);
